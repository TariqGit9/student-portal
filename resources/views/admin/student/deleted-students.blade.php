  ...
  <div class="card">
    <div class="card-header pb-0">
      <div class="d-flex justify-content-between">
        ...
        {{-- <i class="mdi "><span class="float-right" ><button type="button" class="btn btn-secondary" data-toggle="modal" data-target=".addStudentModel">Add a student</button></span></i> --}}
      </div>
    </div>
    <div class="card-body">
...
<script>
        ...
        serverSide: true,
        ajax: {
            url: "{{route('get-deleted-students')}}",
        },
        columns: [
          {
...
$(document).on('click', '.activatestudent', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.post("{{route('activate-student')}}", {
                id: id
            }).then(function(response) {
              toastr.success('Success!', 'student Activated Successfully',{
                "positionClass": "toast-bottom-right"
            })
              datatable.draw();
