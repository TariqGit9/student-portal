...
<div class="breadcrumb-header justify-content-between">
  <div class="my-auto">
    <div class="d-flex">
      <h4 class="content-title mb-0 my-auto">Welcome Super Admin ...</h4>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalTitle"><i class="fe fe-send mr-2"></i>Send Notification to Admins</h5>
...
function renderNotifications(notifications) {
  ...
  if (notifications.length === 0) {
    container.html('<div class="text-center py-5"><i class="fe fe-bell-off tx-50 text-muted"></i><p class="text-muted mt-3 mb-0">No notifications sent yet</p></div>');
    return;
  }
  var html = '';
  notifications.forEach(function(n) {
    ...
    html += '        <small class="text-muted"><i class="fe fe-calendar mr-1"></i>' + n.created_at + '</small>';
    html += '        <span class="badge badge-danger ml-2">Admins</span>';
...
$('#openCreateModal').on('click', function() {
  ...
  $('#notif_expiry').val('');
  $('#modalTitle').html('<i class="fe fe-send mr-2"></i>Send Notification to Admins');
...
$('#saveNotification').on('click', function() {
  ...
  var payload = { title: title, message: message, target_audience: 'admin', expiry_date: expiry };
  if (id) payload.id = id;
